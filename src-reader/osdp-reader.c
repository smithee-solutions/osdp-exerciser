/*
  osdp-reader - PD implementation of OSDP

  (yes includes i/o and i/o only)
...
// set up serial (was called check_serial)
  };

fprintf(stderr, "DEBUG: set up defaults; then read settings; then init serial; then run processing loop\n");
  if (status != ST_OK)
    fprintf(stderr, "osdp-reader: exit status %d\n", status);
  return(status);
